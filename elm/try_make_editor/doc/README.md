...
* [x] 機能: 行頭へ移動 (&lt;HOME&gt; または C-a)
* [x] 機能: 行末へ移動 (&lt;HOME&gt; または C-a)
* [ ] 機能: ページアップ (&lt;PgUp&gt; または M-v)
* [ ] 機能: ページダウン (&lt;PgDn&gt; または C-v)
* [ ] 機能: 行末まで削除 (C-k)
* [ ] 機能: バッファ再読み込み (revert-buffer)
...
* [x] 問題: 計算につかった ruler オブジェクトが下の要素（１行目）のクリックを阻害する
    * 中身を消してもダメ
    * left -9999 とかしてもダメ
    * -> これらは確かに表示は画面外に出るが、クリック判定のあるオブジェクトが残ったままになる
    * ruler ではなくcursor-layer が悪さをしていた。`("pointer-events", "none")` で マウスイベントの対象にして対応した
...
* [x] 不具合: Windows Chrome で IME入力確定を 次の文字入力で始めると, compositionend で textarea.value をクリアしても消えてない（ため、変換候補リストの表示位置がずれる）
    * IME入力切り替えと、textareaのクリア周りを Native に移した.
* [ ] 不具合: 横スクロールしている状態で、マウスクリックでカーソル移動すると場所がずれる
* [ ] 不具合: 範囲選択中のクリックカーソル移動で範囲選択が解除されない
* [x] 不具合: カーソル行のクリックでカーソルが末尾に行かない
    * カーソル行内のバーツ（未確定文字を表示するため、カーソル前後で別の`span`になってる）を `("pointer-events", "none")` で マウスイベントの対象にして対応した
* [x] 不具合: 範囲選択の前側でクリックが発生しないことがある
    * 他の行から映るときに発生する？自の行の後ろからのジャンプなら大丈夫っぽい
    * カーソル行内のバーツ（未確定文字を表示するため、カーソル前後で別の`span`になってる）を `("pointer-events", "none")` で マウスイベントの対象にして対応した
